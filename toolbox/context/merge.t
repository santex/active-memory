...
is ( $sing_count, 4, "Correct count on singular form" );
is ( $plur_count, 8, "Correct count on singular form" );

$g->merge( 'T', 'black truffle', 'black truffles' );

my $combined_count = $g->doc_count( $singular );
is( $combined_count, 12, "Correct combined count" );

# old word should have disappeared
$plur_count = $g->doc_count( $plural );
$sing_count = $g->doc_count( $singular );
is( $plur_count, 0, "Old word disappeared" );
is( $sing_count, 12, "New word has right span" );
...
is( $g->doc_count( $york ), 6, "Correct count for york" );
is( $g->doc_count( $nyc ), 5, "Correct count for NYC" );

$g->merge( 'T', $nyc, $york );
is( $g->doc_count( $nyc ), 6, "Correct updated count for NYC" );
is( $g->term_count(), $pre_merge_count -1, "Global term count went down");
__DATA__
