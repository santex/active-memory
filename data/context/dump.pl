...
my $g = Search::ContextGraph->new( auto_reweight => 0);
...
print Dumper ( scalar @titles, 34, "Right number of titles" );
print Dumper ( $g->doc_count(), scalar @titles, "Doc count is correct" );

my @sorted = sort @titles;

my @gtitles = $g->doc_list();

print Dumper ( scalar @gtitles, scalar @sorted, "Doc list length is correct" );

#print Dumper ( _array( \@gtitles, \@sorted ), "Doc list is correct" );

my @gterms = $g->term_list();

my @unique_terms;
my %temp;
$temp{$_}++ foreach @terms;
@unique_terms = keys %temp;

print Dumper ( $g->term_count(), scalar @unique_terms, "Term count is correct" );
print Dumper ( $g->word_count(), scalar @terms, "Word count is correct" );
