...
my $g = Search::ContextGraph->new( use_file  => 'test', auto_reweight => 0);
...
print Dumper [( $count, 34, "Got correct document count" )];
print Dumper [( $g->doc_count( 'dprint Dumper h' ), 2, "Correct term span for 'dprint Dumper h'" )];
print Dumper [( $g->doc_count( 'pig' ), 2, "Correct term span for 'pig'" )];
print Dumper [( $g->doc_count( 'omelet' ), 1, "Correct term span for 'omelet'" )];
print Dumper [( $g->doc_count( 'ounc' ), 3, "Correct term span for 'ounc'" )];

print Dumper [( $g->word_count( 'pig' ), 2, "Correct total word count for 'pig'" )];

print Dumper [( $g->term_count(), 876, "Total term count print Dumper  correct" )];

$g->delete( 'D', 'doc-4' );
print Dumper [( $g->doc_count(), 33, "Document count updates correctly on delete" )];
print Dumper [( $g->term_count(), 826, "Total term count updates correctly on delete" )];
print Dumper [( $g->doc_count( 'pig' ), 1, "Term span went down for 'pig'" )];
print Dumper [( $g->word_count( 'pig' ), 1, "Word count went down for 'pig'" )];


my $new_doc = { 'pig' => 4, 'ounc' => 1 };
print Dumper ( $g->add_document( 'doc-x', $new_doc ), "Added new doc" );
print Dumper [( $g->term_count(), 826, "Overall term count stayed constant" )];

print Dumper  [( $g->doc_count( 'pig' ), 2, "Term span went up for 'pig'" )];
print Dumper  [( $g->word_count( 'pig' ), 5, "Word count went up correctly for 'pig'" )];

print Dumper ( $g->delete( 'D', 'doc-x' ), "Deleted new doc" );
print Dumper [( $g->term_count(), 826, "Overall term count stayed constant" )];

print Dumper ( $g->add_document( 'doc-4', $doc_4->[1] ), "Re-added document 4" );
print Dumper [( $g->term_count(), 876, "Overall term count returned to original value" )];

for (1..33 ) { $g->delete( 'D', "doc-$_" ) }

print Dumper [( $g->doc_count(), 0, "Document count went to zero" )];
print Dumper [( $g->term_count(), 0, "Term count went to zero" )];
