...
# rebalancing.t - there should be no difference between reweighting the
# graph after each document is added (time consuming) or reweighting
# it after a bunch are added (faster).  This test makes sure of it.
...
my $slow = Search::ContextGraph->new( auto_reweight => 1);
my $fast = Search::ContextGraph->new( auto_reweight => 0);
my $broken = Search::ContextGraph->new( auto_reweight => 0);

ok( $slow, "Created slow graph object" );
ok( $fast, "Created fast graph object" );
ok( $broken, "Created broken graph object" );
...
while ( <DATA> ) {
	...
	$docs{$title} = \@words;
	$slow->add( $title, \@words);
	$fast->add( $title, \@words);
	$broken->add( $title, \@words);
}

ok( $fast->reweight_graph(), "Reweighted graph" );



is( $fast->doc_count(), 34, "Correct doc count on fast" );
is( $slow->doc_count(), 34, "Correct doc count on slow" );
is( $broken->doc_count(), 34, "Correct doc count on broken" );


my ( $s1, $s2 ) = $slow->search( 'pig' );
my ( $f1, $f2 ) = $fast->search( 'pig' );
my ( $b1, $b2 ) = $broken->search( 'pig' );

ok( $f1, "got results for fast" );
ok( $s1, "got results for slow" );
ok( $b1, "got results for broken" );


is_deeply( $f1, $s1, "Equivalent search results on fast and slow" );

my ( $ff1, $ff2 ) = $fast->find_similar( 'doc-10' );
my ( $ss1, $ss2 ) = $slow->find_similar( 'doc-10' );

is_deeply( $f1, $s1, "Equivalent find similar results on fast and slow" );
...
isnt( $v1, $v2, "Fast and broken do not give equivalent results" );

ok( $broken->reweight_graph(), "Reweighting broken graph" );
( $b1, $b2 ) = $broken->search( 'pig' );

is_deeply( $f1, $b1, "Now we have equivalence" );
